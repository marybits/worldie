      ...
      <div className="bg-transparent rounded-2xl border border-gray-800 p-8 sm:p-10 w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <input
            ...
            required
            className="px-3 py-2 rounded-lg bg-gray-800 border border-gray-700 text-gray-100 outline-none focus:border-(--accent) placeholder:text-gray-500"
          />
          <input
            type="email"
            placeholder="Email"
          ...
          <button
            type="submit"
            className="bg-linear-to-br from-(--accent) to-(--accent-dark) text-gray-900 border-none rounded-lg py-2 cursor-pointer font-bold hover:from-(--accent-dark) hover:to-[oklch(32%_0.11_229.7)] hover:scale-105 active:scale-95 transition-all duration-200"
          >
            Register
          </button>
        </form>
        ...
        <p className="mt-4 text-sm">
          ...
          <Link to="/login" className="text-(--accent) hover:text-(--accent-dark)">Login</Link>
        </p>
      </div>
    </div>
  );
